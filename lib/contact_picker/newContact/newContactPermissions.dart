...
askPermission(
  ...
  bool gettingStorage =
      (permissionBeingRequested == PermissionBeingRequested.storage);
  ...
  if (isAuthorized(startStatus) == false) {
    ...
    bool firstTime =
        (gettingStorage) ? storageFirstTime.value : cameraFirstTime.value;
...
class ManuallyChangePermission extends StatefulWidget {
  ...
  @override
  _ManuallyChangePermissionState createState() =>
      _ManuallyChangePermissionState();
}

class _ManuallyChangePermissionState extends State<ManuallyChangePermission>
  ...
  void initState() {
    gettingStorage =
        (widget.permissionBeingRequested == PermissionBeingRequested.storage);
    super.initState();
  ...
  Widget build(BuildContext context) {
    String action =
        (gettingStorage) ? "\"Select a Photo\"" : "\"Take a Photo\"";
      ...
      child: AlertDialog(
        ...
        content: new Text(
          "In order to " +
              ...
              "Enable the " +
              permission +
              " permission in the \"Permissions\" section of \"App Settings\"",
        ),
        actions: <Widget>[
          new FlatButton(
