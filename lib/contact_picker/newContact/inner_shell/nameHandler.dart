    ...
    if (namesFound.length > 0) {
      ...
      if (commaBeforeLast) {
        ...
        if (namesFound.length > 1) {
          ...
          bool commaAfterBeforeLast = (beforeLastName.length > 0 &&
              beforeLastName[beforeLastName.length - 1] == ",");
          if (commaAfterBeforeLast) {
            //remove the comma
            namesFound[lastNameIndex - 1] = namesFound[lastNameIndex - 1]
                .substring(0, beforeLastName.length - 1);
            //0(prefix), 1, 2, 3, 4(suffix)
            resultNames[4] = namesFound[lastNameIndex];
            //remove this name from the list since it's been handled
            namesFound.removeLast();
...
String namesToName(List<FieldData> nameFields) {
  ...
  for (int i = 0; i < names.length; i++) {
    //dr cancel, bubo
    if (prefixPresent && i == (names.length - 1))
      name += ", ";
    else if (i != 0) name += " ";
    name += names[i];
  }
