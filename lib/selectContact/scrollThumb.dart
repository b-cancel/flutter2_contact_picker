...
class _DraggableScrollBarState extends State<DraggableScrollBar> {
  ...
  void initState() {
    ...
    for (int i = 0; i < widget.sortedLetterCodes.length; i++) {
      ...
      //add ourselves
      int ourItemCount =
          widget.letterToListItems[widget.sortedLetterCodes[i]].length;
  ...
  void doMath() {
    ...
    thumbScrollBarHeight =
        widget.visualScrollBarHeight - widget.scrollThumbHeight;

    //determine thumb offset
    if (barOffset <= offsetAtSart)
      thumbOffset = 0;
    else if (offsetAtEnd <= barOffset)
      thumbOffset = thumbScrollBarHeight;
    else {
  ...
  void _onVerticalDragUpdate(DragUpdateDetails details) {
    ...
    //clamp the values
    barOffset =
        barOffset.clamp(0, widget.programaticScrollBarHeight).toDouble();
    ...
    if (newIndex != index) {
      ...
      widget.autoScrollController
          .jumpTo(offsets[index] + widget.expandedBannerHeight);
    }
  ...
  Widget build(BuildContext context) {
    ...
    if (widget.sortedLetterCodes.length == 0)
      thumbTackChar = " ";
    else
      thumbTackChar = String.fromCharCode(widget.sortedLetterCodes[index]);
