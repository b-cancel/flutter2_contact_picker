...
import '../helper.dart';
import 'selectContactLogic.dart';
...
getContactPermission(BuildContext context, bool forcePermission,
    bool selectingContact, Function onSecondaryOption,
    {SelectContactBackUp selectContactBackUp:
        SelectContactBackUp.manualInput}) async {
  PermissionStatus startStatus =
  ...
  if (isAuthorized(startStatus) == false) {
    ...
    if (startStatus == PermissionStatus.notAgain && !contactFirstTime.value) {
      ...
      PermissionStatus status =
          (await Permission.requestPermissions([PermissionName.Contacts]))[0]
              .permissionStatus;
...
class _ManualState extends State<Manual> with WidgetsBindingObserver {
  ...
  final contactPicker.ContactPicker contactPickerInstance =
      new contactPicker.ContactPicker();
  ...
  Widget build(BuildContext context) {
    ...
    bool manualInput =
        (widget.selectContactBackUp == SelectContactBackUp.manualInput);

    String inOrderTo = "In order to " +
        ((isSelecting) ? "\"Select\"" : "\"Save\"") +
        " a Contact.";
    String action =
    ...
    if (isSelecting) {
      if (manualInput)
        options = "Or Manually Input the Contact below";
      else
        options = "Or System Select the Contact below";
    } else
      options = "Or Use the Contact without saving it below";

    String altButton;
    if (isSelecting) {
      if (manualInput)
        altButton = "Manual Input";
      else
        altButton = "System Select Contact";
    } else
      altButton = "Use But Don't Save";
              ...
              onPressed: () async {
                if (isSelecting) {
                  ...
                  if (manualInput) {
                    ...
                    contactPicker.Contact selectedContact =
                        await contactPickerInstance.selectContact();
...
class _ManualInputState extends State<ManualInput> {
  ...
  submitOrRefocus({bool onName: true}) {
    if (nameValid.value && phoneValid.value)
      ...
      if (onName != null && onName == false && phoneValid.value == false)
        return; //stay in phone
      else {
        //focus on the first field that isnt valid
  ...
  submitForm() {
    widget.onSubmit(
        context,
        new Contact(givenName: nameCtrl.text, phones: [
          Item(value: onlyNumbers(phoneCtrl.text), label: "manual")
        ]));
  }
      ...
      child: AlertDialog(
            ...
            children: <Widget>[
              TextField(
                ...
                decoration: InputDecoration(
                    labelText: "Name", helperText: "must not be empty"),
                //on button action
                onEditingComplete: () {
              ...
              TextField(
                ...
                //decoration to indicate field type
                decoration: InputDecoration(
                    labelText: "Phone", helperText: "must be a valid number"),
                //on button action
                onEditingComplete: () {
        ...
        actions: <Widget>[
          ...
          new RaisedButton(
            ...
            onPressed:
                (nameValid.value && phoneValid.value) ? submitForm : null,
          ),
        ],
      ),
    );
