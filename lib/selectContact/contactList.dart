...
class _ContactListState extends State<ContactList> {
  Map<int, List<Widget>> createLetterToWidgetListMap() {
    ...
    for (int i = 0; i < widget.contacts.value.length; i++) {
      ...
      if (contactFullName != "")
        letterCode = contactFullName.toUpperCase()?.codeUnitAt(0);
  ...
  Widget build(BuildContext context) {
          ...
          child: Column(
            ...
            children: <Widget>[
              Container(
                ...
                child: Center(
                  child: Text(
                      widget.contacts.value.length.toString() + " Contacts"),
                ),
              ),
              Container(
                width: MediaQuery.of(context).size.width,
    ...
    //all slivers
    List<Widget> allSlivers = new List.from(widget.headerSlivers)
      ..addAll(bodySlivers);
